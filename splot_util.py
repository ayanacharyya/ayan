...
def readsinglemagespec(inp):
    ...
    wz = np.array(wz)[
        np.logical_and(np.isfinite(fl), np.logical_and(np.less(np.abs(fl), 30.), np.less(np.abs(err_f), 1.)))]
...
def fixcen_gaus2(x, center, center2, *p):
    return p[0] + p[1] * exp(-((x - center) ** 2) / (2 * p[2] ** 2)) + p[3] * exp(
        -((x - center2) ** 2) / (2 * p[4] ** 2))
...
def fixcont_fixgroupz_gaus(x, cont, n, obswave, zz, *p):
    ...
    for xx in range(0, n):
        result += p[2 * xx + 1] * exp(
            -((x - obswave[xx] * (1. + p[0]) / (1. + zz[xx])) ** 2) / (2 * p[2 * xx + 2] ** 2))
    return result
...
def fit(lx, ly, c):
    ...
    sigma = 1.
    if c == 1:
        popt, pcov = curve_fit(gaus1, lx, ly,
                               p0=[ly[0], ly[np.where(np.abs(ly - ly[0]) == np.max(np.abs(ly - ly[0])))[0][0]] - ly[0],
                                   lx[np.where(np.abs(ly - ly[0]) == np.max(np.abs(ly - ly[0])))[0][0]], sigma])
        print 'cont = ' + str(format(popt[0] * const, '.3e')) + ', flux = ' + str(
            format(np.sqrt(2 * np.pi) * popt[1] * popt[3] * const, '.3e')) + \
              ', center = ' + str(format(popt[2], '.3f')) + '\nA, gfwhm = ' + str(
            format(2 * np.sqrt(2 * np.log(2)) * popt[3], '.3e')) + \
              ' A, EW = ' + str(format(np.sqrt(2 * np.pi) * popt[1] * popt[3] / popt[0], '.3e')) + ', core = ' + str(
            format(popt[1] * const, '.3e'))
    elif c == 2:
...
def onpress(event):
    ...
    if event.key == 'c':
        ...
        lx = np.array(lx)
        if c == 1:
            l = 'cont = ' + str(format(popt[0] * const, '.3e')) + ', flux = ' + str(
                format(np.sqrt(2 * np.pi) * popt[1] * popt[3] * const, '.3e')) + \
                ', center = ' + str(format(popt[2], '.3f')) + '\nA, gfwhm = ' + str(
                format(2 * np.sqrt(2 * np.log(2)) * popt[3], '.3e')) + \
                ' A, EW = ' + str(format(np.sqrt(2 * np.pi) * popt[1] * popt[3] / popt[0], '.3e')) + ', core = ' + str(
...
A new figure will pop up with normalised spectra within a region covered by your previously selected regions.\n'
    ...
    for ii in range(i, j + 1):
        ax.axvline(li[ii], ymin=(fl[np.where(wz > list[ii][1])[0][0]] - ax.get_ylim()[0]) / (
...
def makeplotint(wz_full, fl_full, er_full, xmin, xmax, c, list, const, ymin='NONE', ymax='NONE'):
    ...
    mylog.write(
        ' Column format: x1 x2 p[0] p[1] p[2] p[3] where x are clicked wavelengths and p[] are fitted parameters.\n')
    plt.close('all')
